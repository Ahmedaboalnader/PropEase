...
const AuthFormLayout = ({ title, subtitle, children }) => {
    return (
        <div className="w-full min-h-screen flex items-center justify-center p-2 sm:p-4 relative">
            {/* Gradient Background */}
            <div
                className="absolute w-full max-w-xs sm:max-w-sm md:max-w-5xl h-[500px] sm:h-[550px] md:h-[600px] rounded-2xl overflow-hidden shadow-xl bg-transparent -z-50 
                            inset-0 bg-gradient-to-tr from-main to-text transform -translate-x-1/2 -translate-y-1/2 rotate-3"
                style={{
                    top: '50%',
                    left: '50%',
            ...
            <div className="w-full max-w-xs sm:max-w-sm md:max-w-5xl h-[500px] sm:h-[550px] md:h-[600px] relative rounded-2xl overflow-hidden shadow-xl bg-transparent z-10">
                <div
                    className="w-full h-full flex flex-col md:flex-row justify-start rounded-2xl overflow-hidden"
                    style={{
                        backgroundImage: `url(${imageUrl})`,
                        backgroundSize: 'cover',
                        backgroundPosition: 'center',
                ...
                >
                    ...
                    {/* Right side - Form */}
                    <section className="w-full md:w-[80%] md:max-w-[80%] bg-white flex flex-col justify-center items-center rounded-none md:rounded-tl-3xl md:rounded-bl-3xl">
                        {children}
                    </section>
                </div>
            </div>
